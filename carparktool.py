...
class CarParkTool:
    ...
    def reset_account(self) -> bool:
        payload = { "account_auth": self.auth_token }
        response = requests.post(f"{BASE_URL}/reset_account", data=payload)
        response_decoded = response.json()
        return response_decoded.get("ok")

    def set_car_color(self, car_id, color) -> bool:
        payload = { "account_auth": self.auth_token, "car_id": car_id, "color": color }
        response = requests.post(f"{BASE_URL}/set_car_color", data=payload)
        response_decoded = response.json()
        return response_decoded.get("ok")

    def max_upgrade_car(self, car_id) -> bool:
        payload = { "account_auth": self.auth_token, "car_id": car_id }
        response = requests.post(f"{BASE_URL}/max_upgrade_car", data=payload)
        response_decoded = response.json()
        return response_decoded.get("ok")
